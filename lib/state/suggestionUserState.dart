...
class SuggestionsState extends AppState {
  ...
  void setUserlist(List<UserModel>? list) {
    if (list != null && _userlist == null) {
      ...
      list = list.where((user) => user.userId != currentUser?.userId).toList();
      print('Filtered user list: ${list.length}');
      ...
      print('Final user list after filtering: ${_userlist!.length}');
  ...
  bool isFollowing(UserModel user) {
    if (user.followersList != null) {
      print(
          'Checking if current user (${currentUser?.userId}) is in the followers list of ${user.userId}');
      bool isFollowed =
          user.followersList!.any((x) => x == currentUser!.userId);
      print('Is following: $isFollowed');
      return isFollowed;
    } else {
      return false;
    }
