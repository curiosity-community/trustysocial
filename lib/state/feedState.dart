...
class FeedState extends AppState {
  ...
  List<FeedModel>? _filteredFeedList;
  database.Query? _feedQuery;
  List<FeedModel> _tweetDetailModelList = [];

  List<FeedModel>? get feedList {
    // Return filtered list if available, otherwise return main list
    if (_filteredFeedList != null) {
      return List.from(_filteredFeedList!.reversed);
    }
    if (_feedList == null) {
      return null;
    }
  ...
  List<FeedModel>? getTweetList(UserModel? userModel) {
    ...
    if (!isBusy && feedList != null && feedList!.isNotEmpty) {
      list = feedList!.where((x) {
        // Filter out tweets from blocked users
        if (userModel.blockedList != null &&
            userModel.blockedList!.contains(x.user!.userId)) {
          return false;
        }

        // Filter out hidden tweets unless user is the author or a moderator
        if (x.isHidden == true &&
  ...
  /// It makes `Trusty` to view nested Tweets
  set setFeedModel(FeedModel model) {
    if (!_tweetDetailModelList.any((element) => element.key == model.key)) {
  ...
  Future<String?> createTweet(FeedModel model) async {
    ...
    try {
      ...
      if (model.tags != null && model.tags!.isNotEmpty) {
        await updateHashtags(model.tags!);
      }
      return tweetKey;
    } catch (e) {
  ...
  bool isTweetEditable(FeedModel tweet) {
    if (tweet.editedAt != null) return false;
    var tweetTime = DateTime.parse(tweet.createdAt);
    var now = DateTime.now();
    return now.difference(tweetTime).inMinutes <= 5;
  }

  Future<bool> editTweet(FeedModel tweet, String newDescription) async {
    if (!isTweetEditable(tweet)) return false;

    tweet.description = newDescription;
    tweet.editedAt = DateTime.now().toUtc().toString();
    await kDatabase.child('tweet').child(tweet.key!).update({
  ...
  /// Toggle tweet visibility (hide/unhide)
  Future<void> toggleTweetVisibility(FeedModel tweet) async {
    tweet.isHidden = !tweet.isHidden!;
    await updateTweet(tweet);
    notifyListeners();
  }
  ...
  Future<void> updateHashtags(List<String> hashtags) async {
    try {
      for (var tag in hashtags) {
        // Remove '#' and encode the tag to make it Firebase path safe
        String safeTag = tag.replaceAll('#', '').trim();
        if (safeTag.isEmpty) continue;

        var snapshot = await kDatabase.child('hashtags').child(safeTag).once();
        if (snapshot.snapshot.value != null) {
          var count = (snapshot.snapshot.value as Map)['count'] as int;
          await kDatabase
              .child('hashtags')
              .child(safeTag)
              .update({'count': count + 1});
        } else {
          await kDatabase.child('hashtags').child(safeTag).set({
            'tag': tag, // Store original tag with #
            'count': 1,
      ...
      cprint(error, errorIn: 'updateHashtags');
    }
  }

  Future<void> getDataFromDatabaseByHashtag(String hashtag) async {
    try {
      isBusy = true;
      notifyListeners();

      // Filter from existing _feedList instead of making new database call
      if (_feedList != null) {
        _filteredFeedList = _feedList!.where((tweet) {
          return tweet.tags != null &&
              tweet.tags!
                  .any((tag) => tag.toLowerCase() == hashtag.toLowerCase());
        }).toList();
      }
      ...
      _filteredFeedList = null;
      isBusy = false;
      cprint(error, errorIn: 'getDataFromDatabaseByHashtag');
    }
  }

  // Add method to clear filtered list
  void clearFilteredList() {
    _filteredFeedList = null;
    notifyListeners();
  }

  List<FeedModel>? get filteredFeedList => _filteredFeedList;
  ...
  Future<bool> isBookmarked(String tweetId) async {
    ...
    var snapshot =
        await kDatabase.child('bookmark').child(userId!).child(tweetId).once();

    return snapshot.snapshot.value != null;
  }

  /// Remove bookmark
  Future<void> removeBookmark(String tweetId) async {
    final pref = getIt<SharedPreferenceHelper>();
    var userId = await pref.getUserProfile().then((value) => value!.userId);
    await kDatabase.child('bookmark').child(userId!).child(tweetId).remove();
  }
}
